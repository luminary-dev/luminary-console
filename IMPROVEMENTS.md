...
- 2026-08-07 — Wave 6 — independent full-platform QA: 231 checks across auth/OTP/sessions, client lifecycle, money, ops crons, portal UX, assistant + handover pack and cross-cutting (64 page checks at 390/1280 px in both themes, all 9 document PDFs, AI-wording grep, 404s); **24 defects found, 22 fixed** in 10a813d + 1aa389a — headline ones: the unauthenticated 401/redirect skipped harden() entirely; the OTP resend branch sent the browser to the code screen without a pending cookie; `retry-stage2` could replace a published, accepted quotation with an unarchived draft; client deletion ran the irreversible teardown even when the archive email silently failed; the handover pack declared "Account settled in full \u00b7 IP transferred" over contradictory zeroes when no invoice was published, and fell back to the operator's private brief for its client-facing summary; the asset route streamed record.json and the session registry; billing numbers were reusable after deleting the newest document; the proposal lost its phase labels on every phone. Full matrix, every bug with its fix, what was deliberately left, and the owner-only follow-ups are in the Wave 6 section below. eco-mech byte-identical before/after (sha256 2961022\u2026), both throwaway clients and their domains torn down, index back to one entry — 10a813d + 1aa389a
...
fixed in `10a813d` + `1aa389a`, 2 (plus 12 lesser observations) deliberately
...
| B14 | The delete-confirmation password had no rate limit — an 800 ms sleep on the one irreversible endpoint (`1aa389a`) | `rateLimit(req, "auth")` |
...
| B18 | The weekly backup returned `{ok: true}` and logged "ran weekly backup" even when the email never left, so a backup failing for months looked healthy in both the response and the activity feed (`1aa389a`) | 502 on undelivered, and the activity entry says so |
