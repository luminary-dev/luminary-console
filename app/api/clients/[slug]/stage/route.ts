...
export async function POST(
  ...
  // Compare-and-swap so a manual stage override can't be lost to — or clobber —
  // a concurrent auto-advance from a portal accept/payment (AUDIT.md API-02).
  let updated;
  try {
    updated = await updateClient(slug, (client) => {
      client.stage = stage;
