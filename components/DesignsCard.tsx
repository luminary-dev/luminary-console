...
// Add each as a single self-contained HTML file; it gets its own subdomain
// (<slug>-d<id>.luminary-dev.xyz) automatically and stays a draft (holding page
// in public, previewable here) until you publish it. Publish / unpublish /
// delete per slot; delete tears the subdomain down after a confirmation.
import { useCallback, useRef, useState } from "react";
import { useConfirm } from "./ConfirmDialog";
import { MAX_DESIGNS } from "@/lib/designs";
...
export default function DesignsCard({ slug, initial }: { slug: string; initial: DesignEntry[] }) {
  ...
  const remove = async (d: DesignEntry) => {
    const ok = await confirm({
      ...
      message: (
        <>
          <b>{d.title}</b> ({d.domain}) will be removed and its subdomain torn down. This cannot be
          undone.
        </>
      ),
      confirmLabel: "Delete",
  ...
  return (
    <div className="card">
      ...
      <p style={{ color: "var(--muted)", marginTop: 6, fontSize: 13 }}>
        Up to {MAX_DESIGNS} concept previews, each a single HTML file on its own subdomain. Drafts show
        a holding page in public and can be previewed here; publishing makes the subdomain live.
      </p>
      {error && (
      ...
      {designs.length === 0 ? (
        <p style={{ color: "var(--muted)", marginTop: 12, fontSize: 13.5 }}>
          No designs yet. Add one to create its preview subdomain.
        </p>
      ) : (
            ...
            >
              <div style={{ minWidth: 0, flex: "1 1 240px" }}>
                <div style={{ fontSize: 13.5, fontWeight: 600, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  ...
                  {d.dnsStatus !== "automated" && (
                    <span style={{ fontSize: 11, color: "var(--danger, #d33)" }}>DNS: {d.dnsStatus.replace("_", " ")}</span>
                  )}
                </div>
                <div style={{ fontSize: 12, color: "var(--muted)", overflowWrap: "anywhere", marginTop: 2 }}>
                  {d.status === "published" ? (
                    <a href={`https://${d.domain}`} target="_blank" rel="noopener noreferrer">{d.domain}</a>
                  ) : (
